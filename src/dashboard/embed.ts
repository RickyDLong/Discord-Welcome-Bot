...
// ── Per-guild embed update ────────────────────────────────────────────────────

async function updateGuildDashboard(
  client: Client,
  guildId: string,
  ...
  const channel = client.channels.cache.get(statsChannelId) as TextChannel | undefined;
  if (!guild || !channel) {
    console.warn(`[Dashboard] Skipping ${guildDisplayName} — guild or channel not found`);
    return;
  }
  ...
  // Persist member count for REST API
  ...
  const embed = new EmbedBuilder()
    .setColor(0x7c3aed)
    .setTitle(`📊  ${guildDisplayName} — Live Server Stats`)
    .setDescription(
    ...
    .addFields(
      {
        ...
        value: [
          ...
          `🌙 **${idleMembers}** idle`,
          `⛔ **${dndMembers}** dnd`,
          `🎙️ **${guildVoiceSessionCount}** in voice`,
        ].join('  ·  '),
        inline: false,
      },
      {
  ...
  const existing = messages.find(
    m => m.author.id === client.user?.id && m.embeds[0]?.title?.includes('Live Server Stats'),
  );
  ...
  console.log(`[Dashboard] Updated embed for ${guildDisplayName}`);
}

// ── Public entry point — runs for all configured guilds ───────────────────────

export async function updateDashboard(client: Client): Promise<void> {
  // Primary guild (Archix)
  try {
    await updateGuildDashboard(client, config.GUILD_ID, config.STATS_CHANNEL_ID, 'Archix Digital');
  } catch (err) {
    console.error('[Dashboard] Archix update error:', err);
  }

  // Additional guilds from guildConfigs
  for (const cfg of Object.values(GUILD_CONFIGS)) {
    try {
      await updateGuildDashboard(client, cfg.guildId, cfg.statsChannelId, cfg.name);
    } catch (err) {
      console.error(`[Dashboard] ${cfg.name} update error:`, err);
    }
  }
}
