...
// Per-request guild ID — falls back to the bot's primary guild
function gid(req: any): string {
  return (req.query['guild_id'] as string) || config.GUILD_ID;
}

// GET /api/stats/guilds — all guilds the bot has data for
statsRouter.get('/guilds', async (_req, res) => {
...
statsRouter.get('/leaderboard/points', async (req, res) => {
  ...
  const profiles = await resolveProfiles((data ?? []).map(r => r.user_id as string), G);
...
statsRouter.get('/leaderboard/voice', async (req, res) => {
  ...
  const profiles = await resolveProfiles(sorted.map(([uid]) => uid), G);
...
statsRouter.get('/leaderboard/streaks', async (req, res) => {
  const G = gid(req);
...
statsRouter.get('/voice/live', async (req, res) => {
  ...
  const profiles = await resolveProfiles(allUserIds, G);
...
statsRouter.get('/quests/today', async (req, res) => {
  const G = gid(req);
  const today = new Date().toISOString().split('T')[0]!;
...
statsRouter.get('/achievements/recent', async (req, res) => {
  ...
  const { data } = await supabase
    ...
    .limit(limit);
  const profiles = await resolveProfiles([...new Set((data ?? []).map(r => r.user_id as string))], G);
  const enriched = (data ?? []).map(r => ({ ...r, display_name: profiles.get(r.user_id as string)?.name ?? `…${(r.user_id as string).slice(-4)}`, avatar_url: profiles.get(r.user_id as string)?.avatarUrl }));
  res.json(enriched);
});

// GET /api/stats/achievements/leaderboard
statsRouter.get('/achievements/leaderboard', async (req, res) => {
  const G = gid(req);
  const { data } = await supabase
    .from('user_achievements')
...
statsRouter.get('/tiers/distribution', async (req, res) => {
  const G = gid(req);
...
// GET /api/stats/activity/score
statsRouter.get('/activity/score', async (req, res) => {
  const G = gid(req);
  const today = new Date().toISOString().split('T')[0]!;
...
statsRouter.get('/members', async (req, res) => {
  const G = gid(req);
...
// GET /api/stats/economy/leaderboard
statsRouter.get('/economy/leaderboard', async (req, res) => {
  const G = gid(req);
  const { data } = await supabase
    .from('server_economy')
...
// GET /api/stats/economy/overview
statsRouter.get('/economy/overview', async (req, res) => {
  const G = gid(req);
...
async function resolveProfiles(userIds: string[], guild_id: string): Promise<Map<string, UserProfile>> {
  if (userIds.length === 0) return new Map();
  const { data } = await supabase.from('user_profiles').select('user_id, display_name, username, avatar_hash').eq('guild_id', guild_id).in('user_id', userIds);
...
async function resolveNames(userIds: string[], guild_id: string): Promise<Map<string, string>> {
  const profiles = await resolveProfiles(userIds, guild_id);
